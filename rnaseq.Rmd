...
```{r}
# Counts data
counts_data <- read.csv("all_counts.csv", stringsAsFactors = FALSE) %>%
  na.omit() %>%
  group_by(Gene = .[[1]]) %>%
...
top_down_ICS_asthma <- res_asthma_ICS_df[res_asthma_ICS_df$padj < 0.1 & res_asthma_ICS_df$log2FoldChange < -1, ]
...
# Step 1: Filter data for T2 High samples
...
# Step 2: Create DESeq2 dataset for T2 High samples with ICS use vs no ICS use
...
top_down_ICS_T2High <- dds_T2High_ICS_df[dds_T2High_ICS_df$padj < 0.1 & dds_T2High_ICS_df$log2FoldChange < -1, ]
